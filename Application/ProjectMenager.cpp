...
void ProjectMainPage::_OpenProject(const uInt& index) {
	if (index != -1) {
		m_ProjectManagerPtr->m_pProject = reinterpret_cast<Project*>(
...
void ProjectMainPage::_OnButtonEvent(const uInt& command, const uInt& id, [[maybe_unused]] const HWND& hButton) {
	if (command == BN_CLICKED) {
		if (m_ProjectManagerPtr == nullptr)
			Raise("ProjectManager pointer is nullptr");
		...
		case ActionWindow::BUTTON_ADD_TO_LIST:
		{
			Nt::String defaultPath = 
				m_ProjectManagerPtr->m_CreationPage.m_DefaultProjectPath;
